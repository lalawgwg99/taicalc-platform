<template>
  <div class="calculator-shell decision-calculator">
    <section v-if="type === 'car'" class="calc-grid">
      ...
      <ResultPanel eyebrow="真正的車價" label="平均每月總成本" :value="currency(carResult.monthlyTrueCost)">
    ...
    <section v-else-if="type === 'separation'" class="calc-grid">
      ...
      <ResultPanel eyebrow="應結算項目" label="預估結算總額" :value="currency(separationResult.total)">
    ...
    <section v-else-if="type === 'debt'" class="calc-grid">
      ...
      <ResultPanel eyebrow="整合前後" label="整合後月付" :value="currency(debtResult.newPayment)">
    ...
    <section v-else-if="type === 'labor-pension'" class="calc-grid">
      ...
      <ResultPanel eyebrow="退休現金流" label="預估每月年金" :value="currency(laborPensionResult.monthly)">
    ...
    <section v-else-if="type === 'estate-tax'" class="calc-grid">
      <div class="input-panel">
        <div class="seg-control">
          <button :class="['seg-btn', tax.type === 'estate' && 'seg-btn-active']" @click="tax.type = 'estate'">遺產稅</button>
          <button :class="['seg-btn', tax.type === 'gift' && 'seg-btn-active']" @click="tax.type = 'gift'">贈與稅</button>
        </div>
      ...
      <ResultPanel eyebrow="115 年度" label="預估應納稅額" :value="currency(taxResult.tax)">
    ...
    <section v-else-if="type === 'parental'" class="calc-grid">
      ...
      <ResultPanel eyebrow="家庭可領金額" label="合計估算" :value="currency(parentalResult.total)">
    ...
    <section v-else-if="type === 'home-cost'" class="calc-grid">
        ...
        <div class="field-grid">
          ...
          <label>房屋稅率
            <select v-model.number="home.houseTaxRate" class="input-clean">
              ...
              <option :value="2">非自住 2%</option><option :value="3.2">非自住 3.2%</option><option :value="4.8">非自住 4.8%</option>
            </select>
          </label>
      ...
      <ResultPanel eyebrow="不只房貸" label="平均每月持有成本" :value="currency(homeResult.monthlyCost)">
    ...
    <section v-else class="calc-grid">
      ...
      <ResultPanel eyebrow="購買力比較" label="最高實質期末價值" :value="currency(Math.max(...returnResults.map(item => item.realValue)))">
...
const props = defineProps<{ type: 'car' | 'separation' | 'debt' | 'labor-pension' | 'estate-tax' | 'parental' | 'home-cost' | 'real-return' }>();
...
const ResultPanel = defineComponent({
  props: { eyebrow: String, label: String, value: String },
  setup(p, { slots }) { return () => h('aside', { class: 'result-panel' }, [
    h('p', { class: 'result-eyebrow' }, p.eyebrow), h('span', { class: 'result-label' }, p.label),
    h('strong', { class: 'result-total' }, p.value), h('div', { class: 'result-metrics' }, slots.default?.()),
  ]); },
});

const currency = (value: number) => new Intl.NumberFormat('zh-TW', { style: 'currency', currency: 'TWD', maximumFractionDigits: 0 }).format(Number.isFinite(value) ? value : 0);
const percent = (value: number) => `${value >= 0 ? '+' : ''}${(value * 100).toFixed(1)}%`;
const duration = (months: number) => !Number.isFinite(months) ? '無法清償' : `${Math.floor(months / 12)} 年 ${months % 12} 月`;

const car = reactive({ price: 1_000_000, downPayment: 200_000, annualRate: 3, loanYears: 5, years: 8, cc: 1800, fuel: 'gasoline' as const, annualKm: 15_000, efficiency: 14, energyPrice: 31, insuranceAnnual: 25_000, maintenanceAnnual: 15_000, parkingMonthly: 3_000, resaleRate: 35 });
...
@media(max-width:640px){.field-grid{grid-template-columns:1fr}.debt-row,.scenario-row{grid-template-columns:1fr 1fr}.debt-row strong,.scenario-row strong{grid-column:1/-1;padding-bottom:0}.result-panel{position:relative;top:auto}.result-metrics{grid-template-columns:1fr 1fr}}
</style>
